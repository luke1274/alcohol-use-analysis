...
# load packages to complete the assignment
...
```{r overdispersion}
sd(alcohol$alcuse)^2-mean(alcohol$alcuse)
```
So we can consider to use a negative binomial and gamma distribution.  

##3. If we prefer to use a Gaussian distribution, consider using a transformation (e.g., log or square root) to reduce skewness.
```{r transformation}
